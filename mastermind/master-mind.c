...
void digitalWrite(uint32_t *gpio, int pin, int value)
{
  if (value == LOW)
  {
    asm volatile("bic %[gpio], %[pin]" : : [gpio] "r"(gpio), [pin] "r"(1 << pin));
  }
  else
  {
    asm volatile("orr %[gpio], %[pin]" : : [gpio] "r"(gpio), [pin] "r"(1 << pin));
  }
}
...
void pinMode(uint32_t *gpio, int pin, int mode)
{
  if (mode == OUTPUT)
  {
    asm volatile("orr %[gpio], %[pin]" : : [gpio] "r"(gpio + 1), [pin] "r"(1 << (pin * 3)));
  }
  else
  {
    asm volatile("bic %[gpio], %[pin]" : : [gpio] "r"(gpio + 1), [pin] "r"(7 << (pin * 3)));
  }
}
/* send a @value@ (LOW or HIGH) on pin number @pin@; @gpio@ is the mmaped GPIO base address */
...
{
  int exact = 0, contained = 0;
  int used[SEQL] = {0}; // to keep track of indices already matched
  ...
  return (exact << 4) | contained; // Packing exact and contained matches into a single int
}
...
{
  int exact = code >> 4;
  int contained = code & 0xF;

  printf("Exact matches: %d, Contained matches: %d\n", exact, contained);

  if (lcd_format)
  {
    // Code to display matches on LCD (not implemented here)
  }
}
...
void initITimer(uint64_t timeout)
{
  ...
  timer.it_interval.tv_sec = timeout / 1000000;  // seconds
  timer.it_interval.tv_usec = timeout % 1000000; // microseconds
...
void blinkN(uint32_t *gpio, int led, int c)
{
  ...
  {
    // Turn on LED
    digitalWrite(gpio, led, HIGH); // Delay for a short period
    usleep(500000);                // 500ms Turn off LED
    digitalWrite(gpio, led, LOW);  // Delay for a short period
    usleep(500000);                // 500ms
  }
}
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  if (!opt_s)
    initSeq(); // Initialize the secret sequence
  if (debug)
    showSeq(theSeq); // Optionally show the secret sequence
  ...
  while (!found)
  {
    ...
    waitForButton(gpio, pinButton);                  // Wait for a button press
    int buttonPressed = readButton(gpio, pinButton); // Read the pressed button
    attSeq[attempts - 1] = buttonPressed;            // Store the input number in attSeq

    /* Compute the match with the secret sequence, and show the result */
    int match = checkMatch(theSeq, attSeq, attempts); // Compare the sequences
    if (match == attempts)                            // If all elements matched
    {
      found = 1;
      fprintf(stdout, "Sequence found\n");
    }
    else if (attempts == MAX_ATTEMPTS) // If maximum attempts reached
    {
      fprintf(stdout, "Maximum attempts reached\n");
      break;
    }
    else
    {
      fprintf(stdout, "Partial match: %d out of %d\n", match, attempts);
    }
  }
